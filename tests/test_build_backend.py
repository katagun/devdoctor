...
def test_build_command_bundles_static_ui_and_provider_registry() -> None:
    ...
    assert cmd[cmd.index("--name") + 1] == "devdoctor"
    assert cmd[cmd.index("--distpath") + 1] == str(ROOT / "web" / "dist-backend")
    assert f"{ROOT / 'src' / 'devdoctor' / 'data' / 'paths.yaml'}{os.pathsep}devdoctor/data" in cmd
    assert (
        f"{ROOT / 'src' / 'devdoctor' / 'web' / '_static' / 'dist'}"
        f"{os.pathsep}devdoctor/web/_static/dist" in cmd
    )

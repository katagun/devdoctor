...
- **The web UI's lint and test tooling is current: ESLint 10, the React Hooks
  ...
  is fixed. jsdom 30 needs Node 22.22.2+ on 22.x, 24.15+ on 24.x, or 26+, so
  the web CI job and the macOS packaging job now run on Node 24; the web job
  used to take whatever Node the runner shipped, and packaging used Node 20.

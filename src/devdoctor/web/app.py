...
from devdoctor.web.routes_clean import router as clean_router
from devdoctor.web.routes_dashboard import router as dashboard_router
from devdoctor.web.routes_disk_usage import router as disk_usage_router
from devdoctor.web.routes_history import router as history_router
from devdoctor.web.routes_memory import router as memory_router
...
def build_app(
    ...
    *,
    ...
    # to the `devdoctor` logger once; it's idempotent and does not touch
    # uvicorn's own logging config. DEVDOCTOR_VERBOSE=1 raises us to DEBUG.
    configure_logging(_env_flag("DEVDOCTOR_VERBOSE"))
...
def _resolve_static(override: Path | None) -> Path:
    if override is not None:
        return override
    # Bundled built SPA
    with resources.as_file(resources.files("devdoctor.web._static") / "dist") as p:
        return Path(p)
